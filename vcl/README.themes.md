...
lofice VCL (a bunch of  interfaces and a base implementation) has four main platform  specific implementations (gtk, qt, win,
...
the .xcu files) in a file named `registrymodifications.xcu` which lives in `$XDG_CONFIG_HOME/lofice/(somewhere here)`.
...
## The loficeTheme registry flag

```xml
<prop oor:name="loficeTheme" oor:type="xs:short" oor:nillable="false">
  <info>
    <desc>Specifies loficeTheme state.</desc>
    <label>lofice Theme</label>
    ...
```

To enable or disable theming,  we have a `loficeTheme` enum in the registry which is  represented by `enum class ThemeState`.
...
So  in `ColorConfig::SetupTheme()`,  we first  check if  `loficeTheme` enum  is set  to `DISABLED`,  and if  so then  we mark
...
check if `loficeTheme` is set to `RESET` which happens when  the user presses the `Reset All` button (after which he restarts
...
the static StyleSettings instance or not, and if that's true as well, we set `loficeTheme` enum to `ENABLED`

Then in the  last part of `SetupTheme()`,  which we reach only  if `loficeTheme` is set  to `ENABLED`, we check  if the theme
...
When the  `Reset All` button  is pressed, all the  registry color values  are set to  `COL_AUTO` and `loficeTheme` is  set to
`RESET`. Then after restart, the `IsThemeReset` conditional  in `ColorConfig::SetupTheme()` checks if StyleSettings has the system
colors  or not,  and once  it has,  `loficeTheme`  is set  to `ENABLED`  which  then goes  through the  last conditional  and
