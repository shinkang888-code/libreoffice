...
This subdirectory provides support for building lofice as WASM, with the Emscripten toolchain.

You can build lofice for WASM for two separate purposes: 1)
Either to produce a WASM binary of lofice as such, using Qt5 for
its GUI, or 2) just compiling lofice core ("lofice
...
"Building headless lofice as WASM for use in another product".

## Status of lofice as WASM with Qt

Configure `--with-package-format=emscripten` to have `workdir/installation/lofice/emscripten`
...
The build generates a Writer-only LO build. You should be able to run either
    ...
    $ emrun --hostname 127.0.0.1 --serve_after_close workdir/installation/lofice/emscripten/qt_soffice.html
...
Example `bashrc` scriptlet:
    ...
    EMSDK_ENV=$HOME/Development/lofice/git_emsdk/emsdk_env.sh
...
FWIW: it's also possible to build an almost static Linux lofice by just using
...
<https://git.lofice.org/lode/+/ccb36979563635b51215477455953252c99ec013>
...
`workdir/installation/lofice/emscripten/`).  Create another small JS snippet file like
...
- <https://wiki.lofice.io/Development/Emscripten>

Reverted patch:

- <https://cgit.freedesktop.org/lofice/core/commit/?id=0e21f6619c72f1e17a7b0a52b6317810973d8a3e>
...
downloaded and compiled when building lofice.
...
that contains the in-memory file system needed by the lofice
