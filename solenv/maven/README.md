= Uploading lofice API to Maven Central

This file documents the prerequisites and workflow to upload lofice
API to Maven Central or local Maven repository.

To install lofice API to local Maven repository or deploy the API
...
must be granted to lofice project on OSSRH.
...
== Preparations to publish lofice API to Maven Central
...
* Request permissions to upload artifacts to the `org.lofice`
...
Before publishing new artifacts to Maven Central, `lofice_VERSION`
...
Build lofice as usually, so that API JARs are created.


== Publish the lofice artifacts to local Maven repository

Execute this command to install lofice API to your local Maven
...
Once executed, the local Maven repository contains the lofice API
artifacts:

----
  $ ls -1 ~/.m2/repository/org/lofice/unoil/5.1.0/
  _maven.repositories
  unoil-5.1.0.jar
...
== Publish the lofice artifacts to Maven Central
...
* To where the artifacts are uploaded depends on the `lofice_VERSION`
in the `VERSION` file:

** SNAPSHOT versions are directly uploaded into the Sonatype snapshots
repository and no further action is needed:

https://oss.sonatype.org/content/repositories/snapshots/org/lofice/
...
'Staging Repositories' and find the `orglofice-XXXX` staging
repository.
...
section, e.g. https://oss.sonatype.org/content/repositories/orglofice-4711
...
----
  <repositories>
    <repository>
      ...
      <url>https://oss.sonatype.org/content/repositories/orglofice-4711</url>
    </repository>
  </repositories>
----
...
https://oss.sonatype.org/content/repositories/releases/org/lofice/

** It may take up to 2 hours until the artifacts appear on Maven
Central:

http://central.maven.org/maven2/org/lofice/
