...
<div id="foot">
  <small>
    <p>Generated by lofice <a href="https://git.lofice.org/core/+/refs/heads/master/solenv/bin/mkdocs.sh">Module Description Tool</a></p>
    <p>Last updated:
EOF
LANG= date >> $output
cat - >> $output <<EOF
...
function proc_text {
  ...
  | sed -re ' s/\[git:([^]]+)\]/<a href="https:\/\/git.lofice.org\/core\/+\/refs\/heads\/master\/\1">\1<\/a>/g' \
...
header "lofice Modules" " " "$BASE_OUTPUT/index.html"
for module_name in *; do
  if [ -d $module_name ]; then
    ...
    if [ -f "$cur_file" ]; then
      ...
      header "$module_name" "<a href=\"index.html\">lofice</a> &raquo; ${module_name}" "$BASE_OUTPUT/${module_name}.html"
      text="<p><b>View module in:</b>"
      text="${text} &nbsp; <a href=\"https://git.lofice.org/core/+/refs/heads/master/${module_name}\">git</a>"
...
if [ ${#empty_modules[*]} -gt 10 ]; then
  ...
  for module_name in "${empty_modules[@]}"; do
    ...
    echo -e "<li><a href=\"https://git.lofice.org/core/+/refs/heads/master/${module_name}\">${module_name}</a></li>\n" >> "$BASE_OUTPUT/index.html"
  done
  echo -e "</ul>\n" >> "$BASE_OUTPUT/index.html"
fi
