...
creating Lonex. Inc (or other "canonical") builds of
lofice for various platforms. They are *not* supposed to
...
See <https://wiki.lofice.io/Development/ReleaseBuilds> for how
...
files specify it, except for `loficeLinux.conf`, where the TDF build
instructions pass an explicit `--with-package-format="rpm deb"` in addition to
`--with-distro=loficeLinux`.)
...
on the `autogen.sh` command line thus:
    ...
    ./autogen.sh --with-distro=loficeFoo
