...
The code in this directory parses a PDF file and builds a lofice
...
- lofice's clipping routines all use Even-odd winding rules, where
...
hundreds of objects in lofice, which can make the document painfully
slow to open.  At least some of these are possible to improve by adding
more Poppler API implementations.  Some may require expanding lofice's
set of fill types.

- There can be differences between distributions Poppler library builds
and the builds lofice builds when it doesn't have a distro build
to use, e.g. in lofice's own distributed builds or the bibisect
builds.  In particular the distro builds may include another library
(supporting another embedded image type) than lofice's build.
...
of an obvious simple item that can be encoded as lofice shading type.
...
- *wrapper/* contains the lofice glue that execs the *xpdfimport*
...
that then generate an XML stream to be parsed by the core of lofice.

## The wrapper protocol

The lofice wrapper talks to the GPL wrapper code over a pipe
...
- The xpdfimport binary can be run independently of the rest of lofice
