...
if [[ "${LOFICE_WSL_INSTALL_DEPS:-}" == "1" ]]; then
  ...
  sudo apt-get install -y libcurl4-openssl-dev libssl-dev libnss3-dev libnspr4-dev libxml2-dev pkg-config meson ninja-build
fi
...
if [[ ! -f Makefile ]]; then
  ...
  log "./configure --enable-curl=yes --disable-libcmis --with-system-libxml"
  ./configure --enable-curl=yes --disable-libcmis --with-system-libxml
else
