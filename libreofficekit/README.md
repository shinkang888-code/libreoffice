...
loficeKit can be used for accessing lofice functionality
...
LOK functionality can be accessed by including `loficeKit.h[xx]` in your
program.

LOK initialisation (`lok_init`) requires the inclusion of `loficeKitInit.h` in
your program. If you use the C++ `loficeKit.hxx` header, it already includes
`loficeKitInit.h` for you.

(`loficeKit.hxx` is a simple and fully inlined C++ wrapper for the same
functionality as in `loficeKit.h`.)
...
Scanlines are ordered top-down (whereas lofice will internally default
to bottom-up).
...
client and lofice is a two-way channel. The client can initiate an action
by calling the above mentioned methods. The most important methods for the
client -> lofice communication are:
...
1" = 1440 twips). See `lok::Document` in `loficeKit.hxx` for a full list of
methods and their documentation.

The other way around (lofice -> LOK client) is implemented using a
...
`loficeKitCallbackType` enumeration in `loficeKitEnums.h`, the callback
function signature itself is provided by the loficeKitCallback typedef in
`loficeKitTypes.h`. The most important callback types:
...
Introducing explicit new API under `include/loficeKit/` adds type safety but listing each &
